	...
	public void readClientMsg() {
		...
		try {
			...
			if (len > 0) {
				...
				//Send server invitation message back to the asigned player
				else if (type.equals("sInvite")) {
					String user = split[1];
					String ip = "";
					if (ss.checkOnlineUser(user)) // checks if the player is online
					{
						ip = ss.getOnlineUserIp(user);
					}
					/*
					...
					//This String contains identity of invite, the invited user ip
					//and the user name from invitating host
					//then send all to the player, if there match ip then send them the inviation from host
					String msg = "invite_" + ip + this.userName;
					ss.broadcast(msg);
				} 
